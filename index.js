...
async function deleteRow(id, event) {
    ...
    try {
        ...
        const response = await fetch(`https://fsa-crud-2aa9294fe819.herokuapp.com/api/2405-ftb-et-web-ft/events/${id}`, {
            method: "DELETE",
        });
        ...
        if (response.ok) {
            ...
            console.error("Failed to delete entry from the API:", response.statusText);
        }
    } catch (error) {
...
async function addParty(event) {
    ...
    function convertESTToUTC(time24h) {
        ...
        // Handle wrap-around if hours exceed 24
        if (hours >= 24) {
            hours = hours - 24;
        }
